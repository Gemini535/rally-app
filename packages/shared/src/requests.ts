...
export const SubmitComparisonBody = z.object({ sessionId: z.string().uuid(), winnerEntryId: z.string().uuid().nullable() });
export const UpdateEntryBody = z.object({ note: z.string().max(2_000).nullable().optional(), tags: z.array(z.string().max(50)).max(10).optional() }).refine((body) => body.note !== undefined || body.tags !== undefined, 'Provide a note or tags.');
...
export type UpdateEntryBodyInput = z.infer<typeof UpdateEntryBody>;
